...
为实现上述目标，本章提出多星协同防御优化系统\textsc{CoopSatShield}。系统由本地证据抽取（Local Evidence）、抑制型邻居扩散协同（Suppressed Gossip）和逐跳推回执行（Pushback Enforcement）三部分构成。体系结构如图~\ref{fig:ch4_arch}所示。
...
\item \textbf{新增模块}（$\star$~Ch.4）：入端口贡献统计，用于维护每个候选 key 在各入端口的字节统计$\text{bytes}(a,p)$，供后续 pushback 选择上游；证据扩散消息/推回令牌（Evidence Gossip/Pushback Token, EG/PT）解析，用于完成协同消息的轻量级 header 解析与生存时间（Time-To-Live, TTL）递减；seen-set 去重，用 Bloom Filter\citess{bloom1970space}实现 O(1) 消息去重。
\end{enumerate}
...
\item 证据缓存$\mathcal{E}_{v,\tau}$，存储感兴趣区域（Region of Interest, ROI）内接收到的 EG 消息；
...
\caption{\textsc{CoopSatShield}体系结构。(a)单节点内部架构；(b)多节点协同拓扑}
\label{fig:ch4_arch}
\end{figure}
...
\end{equation}
其中$q_{v,e}(\tau)$表示该端口的队列占用（或排队时延估计），$d_{v,e}(\tau)$表示端口丢包率（或随机早期检测/显式拥塞通知，Random Early Detection/Explicit Congestion Notification, RED/ECN 触发率），$Q_{\text{th}},D_{\text{th}}$为阈值。该指标完全来自数据面或交换机 traffic manager 的可观测量，符合星上实时性要求。为了便于后续权重计算，本章进一步定义归一化的拥塞等级：
\begin{equation}
...
\caption{协同控制消息格式与传播示意。(a)EG 消息；(b)PT 消息；(c)EG/PT 传播路径与消息大小对比}
\label{fig:ch4_header}
\end{figure}

图~\ref{fig:ch4_header} 的上、中、下三部分分别对应 EG、PT 以及两类消息在 ISL 上的传播与开销对比。图中的颜色仅用于区分字段功能分组，具体字段顺序、固定头长度和默认 $n=10$ 下的消息大小仍以前文给出的 bit-level 定义为准。
