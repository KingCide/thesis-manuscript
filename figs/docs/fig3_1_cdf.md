...
| M | 良性 key 数 | 攻击 key 数 | 良性 p99 (Mb/s) | 攻击速率范围 (Mb/s) | gap 倍数 |
|---|------------|------------|----------------|-------------------|---------|
| 1 | 376 | 1 | 485 | [10024, 10024] | ×21 |
| 10 | 374 | 10 | 486 | [1000, 1025] | ×2 |
| 100 | 354 | 100 | 495 | [100, 538] | 重叠 |
| 1000 | 54 | 1000 | 65 | [10, 616] | 重叠 |

- 纯良性聚合 p99 = 484.9 Mb/s（fig3_2 校准基线使用此值）
- M=1 时：良性 p99=485 vs 攻击=10024，gap ≈ 21 倍
- M=100 起攻击 key 低端（100 Mb/s）融入良性分布
- M=1000 时良性 key 数骤降至 54：323/377 个良性目的地被攻击 key 占据，剩余 key 全部是低速率（max=70 Mb/s）

**注意**：各子图的良性 p99 随 M 变化——因混合聚合后攻击 key 会"抢走"部分良性目的地（同一目的地同时有良性流和攻击流时归为攻击 key），剩余良性 key 集合不同。fig3_2 校准基线（484.9 Mb/s）使用纯良性流聚合的完整 377 key 集合，与 fig3_1 略有差异但同一量级。

### 可在正文中补充的定量描述

> M=1 时，良性 p99=485 Mb/s 与攻击聚合速率 10024 Mb/s 之间存在约 21 倍的可分性间隙；M=100 时攻击 key 速率下探至 100 Mb/s，与良性分布出现重叠，top-K 检测开始退化。
...
### 2026-02-26：修正文档中过期的 p99 数值

**问题**：文档记录"良性 p99 ≈ 321 Mb/s"，但代码实际输出 ~485 Mb/s，与 fig3_2 校准基线（484.9 Mb/s）一致。

**根因**：321 Mb/s 来自上一版代码/数据，文档未跟随代码更新同步。

**修复**：更新数据表中的 p99 和 gap 倍数为当前实际值（485 Mb/s, ×21）。补充注释说明各子图 p99 随 M 变化的原因（混合聚合时攻击 key 抢占良性目的地）。
